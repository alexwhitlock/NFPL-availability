#!/usr/bin/env python3
"""
fetch_nfpl_grid.py — writes /srv/www/nfpl/availability.json
Run via cron every 30 minutes:
    */30 * * * * /usr/bin/python3 /home/alex/scripts/fetch_nfpl_grid.py
"""
...
OUT_FILE   = Path("/srv/www/nfpl/availability.json")
...
SITE_NAMES = {
    ...
    2782: "Crotch Site 60",
    2783: "Crotch Site 61",
    2784: "Crotch Site 62 - CLOSED DURING LATE SUMMER",
    2785: "Crotch Site 63 - CLOSED DURING LATE SUMMER",
    2786: "Crotch Site 64",
    2787: "Crotch Site 65",
    2788: "Crotch Site 66",
    2789: "Crotch Site 67",
...
LAKE_ORDER = [
    "Big Gull", "Crotch", "Fortune", "Govan", "Granite",
    "Hungry", "Kashwakamak", "Long Schooner", "Mair", "Redhorse", "Round Schooner",
]


def get_lake(name: str) -> str:
    for lake in LAKE_ORDER:
        if name.startswith(lake):
            return lake
    return name.split(" Site")[0].split(" ")[0]


def fetch_avail_dates() -> list[dict]:
    ...
    if outer["iResultCode"] != 0:
        raise RuntimeError(f"API error: {outer['sResult']}")
    return json.loads(outer["sResult"])["listAvailDates"]


def main():
    print(f"[{datetime.now().isoformat()}] Fetching NFPL availability...")
    avail_dates = fetch_avail_dates()

    dates = [d["sDate"] for d in avail_dates]
    avail_map = {d["sDate"]: d["listProductIDs"] for d in avail_dates}

    # Build sorted site list grouped by lake
    sites = []
    by_lake: dict[str, list] = {lake: [] for lake in LAKE_ORDER}
    for pid, name in sorted(SITE_NAMES.items()):
        lake = get_lake(name)
        if lake in by_lake:
            by_lake[lake].append({"id": pid, "name": name, "lake": lake})
        else:
            by_lake.setdefault(lake, []).append({"id": pid, "name": name, "lake": lake})
    for lake in LAKE_ORDER:
        sites.extend(by_lake.get(lake, []))
    ...
    print(f"  Written {len(dates)} dates, {len(sites)} sites → {OUT_FILE}")
